...
class MultiRobotSimulator:
    ...
    def run_decoupled_simulation(self):
        """Run each controller in isolation and display the combined results."""
        # Store original controllers and robots
        original_controllers = self.controllers
        original_controller_names = self.controller_names
        original_colors = [robot.color for robot in self.robots]
        
        # Results storage
        all_results = []
        
        # Run simulation for each controller separately
        for i in range(len(original_controllers)):
            print(f"Simulating {original_controller_names[i]} controller...")
            
            # Reset simulator with just this controller
            self.controllers = [original_controllers[i]]
            self.controller_names = [original_controller_names[i]]
            self.robots = [DifferentialDriveRobot(x=0, y=0, theta=0, color=original_colors[i])]
            self.num_robots = 1
            
            # Reset tracking data
            self.reset()
            
            # Run simulation until end of trajectory
            while self.step():
                pass
            
            # Store results
            results = {
                'name': self.controller_names[0],
                'color': self.robots[0].color,
                'timestamps': self.timestamps.copy(),
                'tracking_errors': self.tracking_errors[0].copy(),
                'control_inputs': self.control_inputs[0].copy(),
                'robot_path': self.robot_paths[0].copy(),
                'avg_error': np.mean(self.tracking_errors[0]) if self.tracking_errors[0] else 0,
                'completion_time': self.timestamps[-1] if self.timestamps else 0
            }
            all_results.append(results)
            
            print(f"  Completed in {len(self.timestamps)} steps, time: {results['completion_time']:.2f}s")
        
        # Now display the combined results
        self.display_decoupled_results(all_results)
        
        # Restore original configuration
        self.controllers = original_controllers
        self.controller_names = original_controller_names
        self.reset()
    def display_decoupled_results(self, all_results):
        """Display the results of decoupled simulations."""
        ...
        ax_sim.set_title('Differential Drive Robots - Controller Comparison (Decoupled)')
        ax_sim.grid(False)
        ...
        ax_error.set_title('Cross-Track Error Comparison')
        ax_error.grid(True)
        ...
        ax_omega = fig.add_subplot(gs[1, 3])
        ...
        traj_x = [point[0] for point in self.trajectory]
        traj_y = [point[1] for point in self.trajectory]
        ax_sim.plot(traj_x, traj_y, 'k-', alpha=0.7, linewidth=2, label='Reference Path')
        
        # Calculate metrics
        avg_errors = []
        completion_times = []
        
        # Plot each controller's results
        for result in all_results:
            name = result['name']
            color = result['color']
            
            # Plot path
            path_x = [point[0] for point in result['robot_path']]
            path_y = [point[1] for point in result['robot_path']]
            ax_sim.plot(path_x, path_y, linestyle='--', color=color, 
                        alpha=0.7, label=f'{name}')
            
            # Plot errors
            ax_error.plot(result['timestamps'], result['tracking_errors'], 
                        color=color, label=f'{name}')
            
            # Plot velocities
            v_values = [inputs[0] for inputs in result['control_inputs']]
            omega_values = [inputs[1] for inputs in result['control_inputs']]
            ax_v.plot(result['timestamps'], v_values, color=color, label=f'{name}')
            ax_omega.plot(result['timestamps'], omega_values, color=color, label=f'{name}')
            
            # Store metrics
            avg_errors.append(result['avg_error'])
            completion_times.append(result['completion_time'])
            
            # Add text for this controller to the simulation plot
            ax_sim.text(0.02, 0.95 - all_results.index(result)*0.05, 
                        f'{color.capitalize()}: {name}', 
                        transform=ax_sim.transAxes, color=color)
        ...
        # Error metrics (sorted)
        error_indices = np.argsort(avg_errors)
        metrics_text += "Average Error (Best to Worst):\n"
        for i, idx in enumerate(error_indices):
            metrics_text += f"{i+1}. {all_results[idx]['name']}: {avg_errors[idx]:.4f} m\n"
        
        metrics_text += "\nCompletion Time (Fastest to Slowest):\n"
        time_indices = np.argsort(completion_times)
        for i, idx in enumerate(time_indices):
            metrics_text += f"{i+1}. {all_results[idx]['name']}: {completion_times[idx]:.2f} s\n"
        
        # Display metrics
        ax_metrics.text(0.1, 0.5, metrics_text, verticalalignment='center')
...
if __name__ == "__main__":
    ...
    # Run decoupled simulation (each controller in isolation)
    sim.run_decoupled_simulation()
